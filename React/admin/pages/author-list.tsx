...
const AuthorListPage = () => {
    ...
    const handleDeleteAuthor = async (id: string) => {
        try {
            ...
            if (!response.ok && response.status !== 200) {
                // Only throw for non-200 responses
                const errorData = await response.json().catch(() => ({}));
                throw new Error(errorData.error || 'Failed to delete author');
            }

            // For both 200 (already deleted) and 204 (successfully deleted) update the UI
    ...
    const handleRestoreAuthor = async (id: string) => {
        try {
            ...
            const response = await fetch(`/api/authors/${id}/restore`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                }
            ...
            if (!response.ok) {
                throw new Error('Failed to restore author');
            }
            ...
            setError('Failed to restore author. Please try again later.');
        }
    };
